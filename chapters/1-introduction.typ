...
It has thus been the focus of many to provide ways to solve fixpoint equations. Most notably Kanster-Tarski theorem @tarski is a key result for deriving the existance of a solution, while Kleene iteration @kleene gives a constructive way to compute them, albeit generally not very efficient. However the mixing of least and greatest fixpoint equations, while greatly increasing the expressiveness, also complicates the search for a solution.
