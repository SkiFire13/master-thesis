...
An alternative intuition for this logic is to see it as splitting the variables $x_i$ and functions $f_i$ of the original system of equations into multiple $x_(i b_1), x_(i b_2), ...$ and $f_(i b_1), f_(i b_2), ...$, with one for each element of the basis. Logic formulas then represent these functions, while moves for an original position $(b, i)$ for player 0 can be seen as partial assignments to the new boolean variables such that $f_(i b) (x_(1 b_1), ...)$ is true. Finally, generating symbolic moves is equivalent to extracting approximately minimal assignments from the formulas representing the functions. We will leave further exploration of this topic to the future, but for our purposes it still provides a nice intuition for how to define logic formulas for our implementations.
