...
TODO: Rework the order of definitions/theorems here.
...
// TODO: Change this, strategy is only memoryless and global strategy is assumed to not be needed.
#definition("strategy")[
...
#definition("strategy induced instance")[
  ...
  Given a starting vertex $v_0 in V_0 union V_1$ an instance also uniquely defines a play, called an induced play, where if $v_i E != emptyset$ then $v_(i+1) = sigma(v_0 v_1 ... v_i)$ if $v_i in V_0$ and $v_(i+1) = tau(v_0 v_1 ... v_i)$ if $v_i in V_1$, otherwise the play is finite and stops at $v_i$.
]
...
// TODO: Cite papers on determinacy (see Jurdzinski) (is this still a TODO?)
By the well-known determinacy of parity games @pg_ermeson @pg_zielonka we know that each vertex is winning for exactly one of the two players. Moreover it is known that the winning player also has a memoryless winning strategy, that is a strategy that depends only on the current vertex and not on the previous ones. The strategy iteration algorithm will focus on these strategies, as they are much simplier to represent and search for.

#lemma("determinacy of parity games")[
  Given a parity game $G = (V_0, V_1, E, p)$, for every vertex $v in V_0 union V_1$ one and only one of the players has a winning strategy from $v$. The set of vertices $V$ can thus be partitioned in two *winning sets* $W_0$ and $W_1$ of the vertices where player 0 (resp. player 1) has a winning strategy starting from vertices in that set.
]
